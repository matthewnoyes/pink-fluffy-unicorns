...
public class NewsScrapper{
	...
	private class newsObj{  // possibly in another file ??
		...
		public newsObj(String time, String title, String url,String source){ // for alliance news as it doesnt have an impact %
			this.DateTime = time;
			this.title = title;
			this.url = url;
			this.source = source;
	...
	public ArrayList<newsObj> getRnsNews(String company) throws IOException {
	...
	private void rnsNewsArticleGetter(String webpage) throws IOException {
	...
	public ArrayList<newsObj> getAllianceNews(String company) throws IOException { // COMPANY OFFICIAL NAME E.G BARCLAYS = BARC MUST BE IN CAPITALS LETTERS
			...
			for(Element li: doc.select("ul li")){ // going through each list item and adding the article into the arraylist
				...
				articlesObjs.add(newsArrayAdder(time,title,url,"London stock exchange - Alliance News"));  //articles are added to the array list as objects.
			}
		return articlesObjs;
	}

	public ArrayList<newsObj> getYahooNews(String comapny) throws IOException{
		String companyName = "";
		if(comapny.charAt(comapny.length() -1) == '.'){ // This is needed at the word company must end with .l e.g. barc = barc.l
			...
			for(Element li: doc.select("item")){
				System.out.println(li.select("title").text()+ "      " + li.select("link").text() + li.select("pubDate").text());
				final String date = li.select("pubDate").text();
	...
	public ArrayList<newsObj> sectorNews(String sector) throws IOException {
			...
			*/			
			...
			for(Element li: doc.select("#stream li")){ // going through each list item and adding the article into the arraylist
				if(li.select(".stream-card__date .o-date.o-teaser__timestamp").text().length() >0){  // needed to prevent empty spaces from being added due to ads on the website
					...
					String dateTime = li.select(".o-teaser__timestamp").first().attr("datetime"); // the datetime of the article.
					final String relHref = "https://www.ft.com" + li.select(".js-teaser-heading-link").first().attr("href");
