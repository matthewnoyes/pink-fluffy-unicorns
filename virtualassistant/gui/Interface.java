...
private void makeButtons() {
		...
		VBox updateDetails = new VBox(0);
		updateDetails.setAlignment(Pos.BOTTOM_RIGHT);

		Image wifiImage = new Image(getClass().getResourceAsStream("images/wifi_no_access.png"));
		wifiImageView = new ImageView(wifiImage);
		wifiImageView.setPreserveRatio(true);
		wifiImageView.setFitHeight(30);

		update_time = new Label("Last updated: ...");
		update_time.setId("update_time");

		updateDetails.getChildren().addAll(wifiImageView, update_time);

		updateContain.getChildren().add(updateDetails);
